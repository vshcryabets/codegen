...
interface PrepareFilesListUseCase {
    operator fun invoke(root: Node, projectOut: OutputTree): Map<FileGenerator.BlockPath, List<FileData>>
}
...
class PrepareFilesListUseCaseImpl @Inject constructor(
    ...
    override fun invoke(root: Node, projectOut: OutputTree): Map<FileGenerator.BlockPath, List<FileData>> {
        val files = mutableMapOf<FileGenerator.BlockPath, List<FileData>>()
        prepareFilesByTree(projectOut, root, files)
        return files
    }

    private fun prepareFilesByTree(projectOut: OutputTree, root: Node, files: MutableMap<FileGenerator.BlockPath, List<FileData>>) {
        root.subs.forEach {
            if (it is Block) {
